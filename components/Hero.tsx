...
function StatChip({
  ...
  return (
    <div
      className="absolute flex items-center gap-3 rounded-md bg-surface p-[14px_18px] shadow-pop"
      style={{ ...style, animation: "bob 6s ease-in-out infinite", animationDelay: `${delay}s` }}
    >
      <div
        aria-hidden="true"
        className={`${skin} grid size-10 shrink-0 place-items-center rounded-md font-serif text-[16px] font-bold shadow-clay`}
      >
        {icon}
      </div>
      <div>
        <div className="font-serif text-[20px] leading-6 font-bold text-ink">{value}</div>
  ...
  return (
    ...
    >
      <div className="min-w-0 flex-[1.1]">
        ...
        <h1 className="t-hero my-[26px_18px] text-ink">
          Hi, I&apos;m Usama.
